...
**≤264-byte stream compressor with drift-aware RLE-XOR—halves MCU UART bandwidth on repetitive logs.**

## Specs  
- **Flash**: 252 B (M4) / 264 B (M0+) @ `-Os -mthumb -nostdlib`  
- **RAM**: 36 B state (no heap, no initialized data)  
- **Compression**: 40-60 % on *repetitive* firmware traces (status lines, hex dumps, etc.)  
  – Plain English or random data: 0-10 % expansion  
